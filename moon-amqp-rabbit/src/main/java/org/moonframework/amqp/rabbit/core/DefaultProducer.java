 ...
 * @author quzile
 */
public class DefaultProducer extends AbstractProducer<DefaultProducer.Status, Throwable> {
    ...
    protected void doRecover(RetryContext context) throws Exception {
        Message message = (Message) context.getAttribute(AbstractProducer.MESSAGE);
        messageRecovery.recover(context.getRetryCount(), message, context.getLastThrowable());
    }
